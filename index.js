  ...
  socket.on("newRoom", (name, roomName) => {
    if (roomName in data) {
      socketIO
        ...
        .emit(
          "newRoomDeclined",
          "Room with this name already exists! Use another name"
        );
    } else {
      ...
      if (roomName in data) {
        if (data[roomName][0].includes(name)) {
          if (data[roomName][0][0] === name) {
            ...
            socket.emit(
              "newUserAccepted",
              `Admin joined ${roomName} room successfully!!`,
              data[roomName][0]
            );
          } else {
            socket.emit(
              "newUserDeclined",
              "User name already taken! Join with another username."
            );
          }
        } else {
          socket.join(roomName);
          ...
          socket.emit(
            "newUserAccepted",
            `You joined ${roomName} room successfully!!`,
            data[roomName][0]
          );
        ...
        socket.emit(
          "newUserDeclined",
          "Room does not exist!! Check the Room Code again."
        );
      }
    } catch (error) {
      console.log(error);
  ...
  socket.on("getWinnerName", (roomName) => {
    try {
      ...
      const sortedScores = Object.fromEntries(
        Object.entries(scores[roomName]).sort(([, a], [, b]) => b - a)
      );
